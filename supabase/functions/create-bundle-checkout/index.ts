...
serve(async (req) => {
  ...
  try {
    ...
    // Get the bundle ID from the request body
    ...
    // Fetch the bundle to get price info
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      mode: "payment",
      success_url: `${origin || DEFAULT_ORIGIN}/meine-formulare?bundle=${bundle.slug}&success=true`,
      cancel_url: `${origin || DEFAULT_ORIGIN}/bundles/${bundle.slug}`,
      metadata: {
        bundle_id: bundleId,
        user_id: user.id,
